...
sudo -E bash -c 'echo '\''DOCKER_OPTS="-H tcp://0.0.0.0:4243 -H unix:///var/run/docker.sock --storage-driver=aufs --registry-mirror https://6udu7vtl.mirror.aliyuncs.com"'\'' >> /etc/default/docker'
